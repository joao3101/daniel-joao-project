...
var (
	teamRoundDBTypes = map[string]string{`ID`: `int`, `RoundID`: `int`, `TeamID`: `int`, `Score`: `int`, `CreatedAt`: `timestamp`, `DeletedAt`: `timestamp`}
	_                = bytes.MinRead
)
...
func testTeamRoundsUpsert(t *testing.T) {
	...
	if len(mySQLTeamRoundUniqueColumns) == 0 {
