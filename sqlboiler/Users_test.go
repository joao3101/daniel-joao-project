...
func testUserToManyUserTeams(t *testing.T) {
	var err error
	...
	slice := UserSlice{&a}
	if err = a.L.LoadUserTeams(tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.UserTeams); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.UserTeams = nil
	if err = a.L.LoadUserTeams(tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.UserTeams); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyAddOpUserTeams(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		...
		if a.R.UserTeams[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.UserTeams[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.UserTeams().Count(tx)
		if err != nil {
			t.Fatal(err)
		}
...
var (
	userDBTypes = map[string]string{`ID`: `integer`, `FullName`: `character varying`, `Email`: `character varying`, `HashedPassword`: `character varying`, `CreatedAt`: `timestamp without time zone`, `DeletedAt`: `timestamp without time zone`}
	_           = bytes.MinRead
)
